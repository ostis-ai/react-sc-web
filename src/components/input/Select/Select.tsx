...
import OpenStatusButton from '@assets/images/chevronDown.svg';
...
const findAbledOptionValue = (options: PartialOption[]) =>
...
export interface SearchSelectProps
  ...
  /**
   * @deprecated TODO: remove when sugest field is done
   * */
...
export const Select = forwardRef<HTMLSelectElement, SearchSelectProps>(
  (
    {
      value,
      isLoading,
    ...
    const dropDownChildren = useMemo(() => {
      if (isLoading && loadingAppearance === 'options')
        return <DropdownOption>{loadingMessage}</DropdownOption>;
      return (
        <>
    ...
    // const { containerRef: containerRefFromProps } = props;
    ...
    const onConstantOptionUnMount = useCallback(
      (option: IConstantOption) =>
        setConstantOptions((prev) =>
          prev.filter((prevOption) => prevOption.value !== option.value),
        ),
      [],
    );
    ...
    const renderedDefaultSelectValue = multiple
      ? renderMultipleSelectValue()
      : renderedSelectedOption;
    const visibleValue =
      renderedSelectValue ||
      renderedDefaultSelectValue ||
      (renderedEmptyValue ?? localValue) ||
      null;
    ...
    const wrappedVisibleValue = visibleValueIsString ? (
      <div className={styles.stringValueWrapper}>{visibleValue}</div>
    ) : (
      visibleValue
    );
    ...
    const scrollToOption = (optionValue: string) => {
      ...
      const optionElem = dropDownOptions.find((option) => option.value === optionValue)?.ref
        ?.current;
    ...
    const findNextHoverOptionValue = useCallback(() => {
      const nextAbledOptionValue = findAbledOptionValue(
        dropDownOptions.slice(hoverOptionIndex + 1),
      );
    ...
    const findPrevHoverOptionValue = useCallback(() => {
      ...
      const prevAbledOptionValue = findAbledOptionValue(
        dropDownOptions.slice(0, sliceInd).reverse(),
      );
    ...
    const narrowSelectValueToInputValue = (evt: React.KeyboardEvent) => {
      if (
        !visibleValueIsString ||
        !inputRef.current ||
        searchValue ||
        !shouldRenderSelectValue ||
        !localValue
      )
        return;
    ...
    return (
      <div
        className={classNames(className, styles.selectWrapper, {
          [styles.selectWrapper_focused]: isFocused,
          [styles.selectWrapper_disabled]: disabled,
          [styles[`selectWrapper_${dimention}`]]: dimention,
          [styles[`selectWrapper_${appearance}`]]: appearance,
          [styles[`selectWrapper_${status}`]]: status,
        })}
        style={style}
        ref={containerRef}
        onKeyUp={handleKeyUp}
      ...
      >
        ...
        {iconsLeft && <div className={styles.iconsLeft}>{iconsLeft}</div>}
        <div
          className={classNames(styles.valueWrapper, {
            [styles.valueWrapper_disabled]: disabled,
            [styles.valueWrapper_shiftChips]: multiple && !isEmpty,
            [styles.valueWrapper_multiple]: multiple,
            [styles.valueWrapper_fixHeight]: shouldFixHeight,
          })}
        ...
        <div className={styles.iconPanel} onClick={stopPropagation} onMouseDown={preventDefault}>
          {iconsRight && isDeleteButton && (
            <div className={styles.iconClose} onClick={handleClickCloseIcon}>
              {iconsRight}
            </div>
          )}
          {isLoading && loadingAppearance === 'input' && (
            <Spinner size={22} appearance={SPINER_COLOR} />
          )}
          <OpenStatusButton
            className={classNames(styles.openStatusButton, {
              [styles.openStatusButton_opened]: isSearchPanelOpen,
            })}
