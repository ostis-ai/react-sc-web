...
import { TLinkFormat } from '@components/ScnBase/model';
...
import { Tooltip } from 'ostis-ui-lib';
...
const HtmlNodeToReact = ({ node }: IHtmlNodeToReactProps) => {
  ...
  if (node.nodeName === 'SC_ELEMENT') {
    const systemId =
      (node as HTMLElement).attributes.getNamedItem('sys_idtf')?.nodeValue || undefined;
...
const ScLinkHtml = ({ addr }: IProps) => {
  ...
  return (
    <StyledScTag isHTML addr={addr}>
      {contentHtml &&
        Array.from(contentHtml.childNodes).map((childNode, ind) => (
          <HtmlNodeToReact key={ind} node={childNode} />
        ))}
  ...
  return (
    <Tooltip title={content}>
      <StyledScTag forwardedAs="span" addr={addr}>
        {content && contentType === 'format_png' && (
